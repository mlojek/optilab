...
class DifferentialEvolution:
    ...
    def initialize_population(self) -> None:
        ...
        """
        self.population = [
            [random.uniform(*self.bounds) for _ in range(self.dimensions)]
    ...
    def evaluate_population(self) -> None:
        ...
        """
        self.evaluations = [self.evaluate_specimen(vec) for vec in self.population]
        if self.best_value:
    ...
    def evaluate_new_population(self) -> None:
        ...
        """
        self.new_evaluations = [
            self.evaluate_specimen(vec) for vec in self.new_population
        ]
    ...
    def selection(self) -> None:
        ...
        """
        final_population = []
        for curr_vec, curr_val, new_vec, new_val in zip(
            self.population, self.evaluations, self.new_population, self.new_evaluations
        ):
            if new_val:
                if new_val < curr_val:
    ...
    def mutate(self) -> None:
        ...
        for specimen in self.population:
            ...
            random_to_random = [
                a - b
                for a, b in zip(
                    random.choice(self.population), random.choice(self.population)
                )
            ]
            new_specimen = [
                a + self.mutation_factor * (b + c)
                for a, b, c in zip(specimen, current_to_best, random_to_random)
            ]
...
if __name__ == "__main__":
    ...
    optimal_solution = de.optimize(f1, 10 * 1e4, 4 * 10, 100, 1e-8, 10, 5, 0.001, 0.1)
